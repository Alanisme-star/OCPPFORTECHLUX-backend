...
def cancel_ws_disconnect_cleanup(cp_id: str):
    ...
    # ==================================================
    # 3) 清掉舊的 prepare feedback task（若殘留）
    #    目前 prepare feedback 已停用，這裡只做被動清理，不再視為正式流程
    # ==================================================
    pending_ws_prepare_feedback_tasks.pop(cp_id, None)
...
async def finalize_ws_prepare_feedback_cleanup(
    ...
    先前此函式會在「無 active transaction」時做延後清理，
    但這會讓刷卡未插槍的實體樁回綠燈時間被拉長，
    並且在 reconnect / disconnect 反覆時造成 delay task 重排。

    現在策略改為：
    - 無 active transaction：在 websocket_endpoint().finally 直接 immediate cleanup
    - 本函式保留名稱，僅避免舊程式碼誤呼叫時出錯
    """
    try:
        logger.warning(
            f"[WS_PREPARE_FEEDBACK][LEGACY_NOOP] "
            f"cp_id={cp_norm} | expected_seq={expected_seq}"
        )
    except Exception as e:
        logger.exception(
            f"[WS_PREPARE_FEEDBACK][LEGACY_NOOP_ERR] cp_id={cp_norm} | err={e}"
        )
    finally:
...
async def websocket_endpoint(websocket: WebSocket, charge_point_id: str):
    ...
    try:
        ...
        # ==================================================
        # 關鍵保護：
        # 若這個 finally 對應的是舊 WebSocket 物件，而目前已存在更新的連線物件，
        # 則這個 stale finally 不准再介入狀態處理，避免覆蓋新連線狀態。
        # ==================================================
        latest_cp = connected_charge_points.get(cp_norm)
        if current_cp is not None and latest_cp is not None and latest_cp is not current_cp:
            logger.warning(
                f"[WS_DISCONNECT][FINALLY_SKIP][STALE_CP_INSTANCE] "
                f"cp_id={cp_norm}"
            )
            return
        ...
        try:
            ...
            if not active_row:
                ...
                if connected_charge_points.get(cp_norm) is current_cp:
                    connected_charge_points.pop(cp_norm, None)
                ...
                try:
                    ...
                    logger.exception(
                        f"[WS_DISCONNECT][NO_ACTIVE_TX][DB_LOG_ERR] cp_id={cp_norm} | err={e}"
                    )
                ...
                logger.warning(
                    f"[WS_DISCONNECT][NO_ACTIVE_TX][IMMEDIATE_AVAILABLE] "
            ...
            #    → 才進入 grace 保護
            # ==================================================
            if current_cp is not None and connected_charge_points.get(cp_norm) is not current_cp:
                logger.warning(
                    f"[WS_DISCONNECT][GRACE_SKIP][STALE_CP_INSTANCE] cp_id={cp_norm}"
                )
                return
...
def _calculate_multi_period_cost_detailed(transaction_id: int):
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        # 💡 效能優化：一次性把所有日期的規則讀出來，避免在迴圈內不斷 Query
        cur.execute("SELECT date, start_time, end_time, price FROM daily_pricing_rules")
        all_rules = cur.fetchall()

    if len(rows) < 2:
        return {"total": 0.0, "segments": []}

    # 將規則轉為字典方便查詢 dict[date] = [(start, end, price), ...]
    rules_dict = {}
    for r_date, r_start, r_end, r_price in all_rules:
        if r_date not in rules_dict:
            rules_dict[r_date] = []
...
class ChargePoint(OcppChargePoint):
    ...
    async def on_start_transaction(
        ...
        try:
            ...
            # ==================================================
            try:
                request_rebalance(reason="start_transaction")
            except Exception as e:
                logging.exception(
    ...
    async def on_stop_transaction(self, **kwargs):
        ...
        # ==================================================
        try:
            request_rebalance(reason="stop_transaction")
        except Exception as e:
            logging.exception(
...
def get_community_dashboard():
    """
    社區管委會總覽 Dashboard API

    注意：
    - 只做資料彙整與前端顯示用
    - 不修改 OCPP WebSocket / 交易 / Smart Charging / 推播流程
    """
    ...
    def _get_live_power_kw(cp_id: str):
        live = live_status_cache.get(cp_id, {}) or {}

        # 依照目前專案不同版本可能出現的 key 做相容
        for key in [
            "power_kw",
            "current_power_kw",
            "powerKw",
            ...
            if key in live and live.get(key) is not None:
                ...
                # 若 power 是 W，做保守轉換；若本來就是 kW，直接用
                if key == "power" and value > 100:
    ...
    def _get_live_energy_kwh(cp_id: str):
        live = live_status_cache.get(cp_id, {}) or {}
        ...
        for key in [
            ...
            if key in live and live.get(key) is not None:
                return _round2(live.get(key))

        return 0.0

    def _get_live_estimated_amount(cp_id: str):
        live = live_status_cache.get(cp_id, {}) or {}

        for key in [
            "estimated_amount",
            "estimatedAmount",
            "summaryEstimatedAmount",
            "cost",
            "live_cost",
            "liveCost",
        ]:
    ...
    def _get_current_status(cp_id: str):
        st = charging_point_status.get(cp_id, {}) or {}
    ...
    def _is_abnormal_status(status: str):
    ...
    try:
        with get_conn() as conn:
            ...
            if tx_cp_col and tx_stop_col:
                ...
                order_col = tx_start_col or tx_id_col or tx_cp_col

                cur.execute(f"""
                    SELECT {", ".join(active_select_cols)}
                    FROM transactions
                    WHERE {tx_stop_col} IS NULL
                    ORDER BY {order_col} DESC
                """)
                active_rows = cur.fetchall()

                for row in active_rows:
                    cp_id = _normalize_cp_id(str(_row_get(row, tx_cp_col, "") or ""))
                    card_id = str(_row_get(row, tx_card_col, "") or "") if tx_card_col else ""
                    ...
                    if tx_energy_col:
                        ...
                        meter_start = _safe_float(_row_get(row, tx_meter_start_col, 0), 0)
                        meter_stop = _safe_float(_row_get(row, tx_meter_stop_col, 0), 0)
                        if meter_stop > meter_start:
                            energy_kwh = round((meter_stop - meter_start) / 1000.0, 2)

                    # 進行中交易若 DB 度數還沒寫入完整值，就用 live cache 補
                    if energy_kwh <= 0:
                        energy_kwh = _get_live_energy_kwh(cp_id)

                    estimated_amount = _get_live_estimated_amount(cp_id)
            ...
            # ==================================================
            # 5) Alerts：離線 / Unavailable / Faulted
            # ==================================================
            for cp_id, cp_info in cp_map.items():
                status = _get_current_status(cp_id)
                status_lower = status.lower()

                if status_lower in {"offline", "unknown"} or (
                    cp_id not in connected_charge_points
                    and status_lower not in {"charging", "preparing"}
                ):
                    alerts.append({
                        "type": "offline",
                        "level": "warning",
                        "message": f"{cp_info.get('chargePointName') or cp_id} 目前可能離線",
                        "chargePointId": cp_id,
                        "residentName": "",
                    })
                elif status_lower in {"unavailable"}:
                    alerts.append({
                        "type": "unavailable",
                        "level": "warning",
                        "message": f"{cp_info.get('chargePointName') or cp_id} 目前為 Unavailable",
                        "chargePointId": cp_id,
                        "residentName": "",
                    })
                elif status_lower in {"fault", "faulted"}:
                    alerts.append({
                        "type": "fault",
                        "level": "danger",
                        "message": f"{cp_info.get('chargePointName') or cp_id} 回報 Faulted",
                        "chargePointId": cp_id,
                        "residentName": "",
                    })
        ...
        logging.exception(f"❌ get_community_dashboard failed: {e}")
        raise HTTPException(status_code=500, detail=f"get_community_dashboard failed: {str(e)}")

    # alerts 不要一次回太多，避免首頁過長
    alerts = alerts[:20]

    return {
        "summary": summary,
        "chargingNow": charging_now,
        "alerts": alerts,
